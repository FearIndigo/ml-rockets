...
{
    /// <summary>
    /// The way the GridSensor2D process detected colliders in a cell.
    /// </summary>
    public enum Process2DCollidersMethod
    {
        /// <summary>
    ...
    /// Grid-based sensor.
    /// </summary>
    public class GridSensor2DBase : ISensor, IDisposable
    {
        string m_Name;
        Vector2 m_CellScale;
        ...
        )
        {
            ...
            m_NumCells = m_GridSize.x * m_GridSize.y;
            m_CellObservationSize = GetCellObservationSize();
            m_ObservationSpec = ObservationSpec.Visual(m_CellObservationSize, m_GridSize.x, m_GridSize.y);
            m_PerceptionTexture = new Texture2D(m_GridSize.x, m_GridSize.y, TextureFormat.RGB24, false);
        ...
        protected internal virtual Process2DCollidersMethod GetProcess2DCollidersMethod()
        {
            return Process2DCollidersMethod.ProcessClosestColliders;
        }
        ...
        internal void ProcessDetectedObject(GameObject detectedObject, int cellIndex)
        {
            ...
            for (var i = 0; i < m_DetectableTags.Length; i++)
            {
                ...
                {
                    if (GetProcess2DCollidersMethod() == Process2DCollidersMethod.ProcessAllColliders)
                    {
        ...
        {
            int index = 0;
            for (var h = m_GridSize.y - 1; h >= 0; h--)
            {
